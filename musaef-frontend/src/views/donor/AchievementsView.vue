<template>
  ...
  >
    ...
    <main class="container-fluid px-2 px-md-4">
      ...
      <div v-if="loading && !cardData.donor_code" class="text-center py-5">
      ...
      <template v-else>
        ...
        <div
          class="card border-0 rounded-4 shadow-lg p-3 p-md-4 mb-3 mb-md-4 text-white donor-red-card position-relative overflow-hidden"
        >
          <div
            class="d-flex align-items-center justify-content-between flex-column flex-md-row flex-xl-row gap-3 gap-md-4"
          >
            <!-- قسم فصيلة الدم المربوطة حياً بالبيانات الصحية والمستوى -->
            <div
              class="blood-type-section d-flex flex-column align-items-center justify-content-center text-center px-2"
            >
              <div class="d-flex align-items-center gap-2 mb-1 justify-content-center">
                <div class="text-center">
                  <small class="text-white-50 fs-9 d-block text-center">{{
                    t("bloodType")
                  }}</small>
                  <h2
                    class="fw-black text-white mb-0 lh-1 fs-3 fs-md-2 text-center"
                    dir="ltr"
                  >
                    {{ donorBloodType }}
                  </h2>
                ...
                <img
                  :src="getIconUrl('blood-icon.png')"
                  alt="قطرة"
                  class="blood-drop-header-img"
              ...
              <div class="text-center w-100">
                <small class="text-white-50 fs-9 d-block mb-0.5 text-center">{{
                  t("currentLevel")
                }}</small>
                <span class="fw-bold text-white fs-8 d-block text-center">{{
                  translateLevel(cardData?.level)
                }}</span>
              </div>
            </div>
            ...
            <!-- اسم المتبرع والموقع -->
            <div
              class="text-center text-md-start ps-xl-3 flex-grow-1 w-100 w-md-auto"
              :class="currentLanguage === 'ar' ? 'text-md-end' : 'text-md-start'"
            >
              <h3 class="fw-black text-white mb-1 fs-5 fs-md-4 text-truncate">
                {{ donorName }}
              </h3>
            ...
            <!-- صورة المتبرع المحدثة حياً داخل المستطيل الأحمر -->
            <div class="donor-avatar-center-wrapper text-center my-1 my-lg-0 mx-auto">
              <div class="position-relative d-inline-block avatar-gold-container">
                ...
                />
                <div
                  class="avatar-heart-badge position-absolute bottom-0 start-50 translate-middle-x rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                >
            ...
            <div
              class="d-flex flex-column align-items-center text-center flex-grow-1 pe-xl-3 w-100 w-md-auto"
              :class="
                currentLanguage === 'ar'
                  ? 'align-items-md-end text-md-end'
                  : 'align-items-md-start text-md-start'
              "
            >
              <h4 class="fw-bold text-white mb-1 fs-6 fs-md-5">
                {{ t("smartCardTitle") }}
              </h4>
              <small class="text-white-50 fs-9 d-block mb-1">{{ t("donorCode") }}</small>
              <strong class="fs-7 font-monospace tracking-wider mb-2 text-white">{{
                cardData?.donor_code || "BD123456789"
              }}</strong>

              <div
                class="badge-status-pill border border-warning border-opacity-75 rounded-pill px-3 py-1 fs-9 fw-bold text-warning d-inline-flex align-items-center gap-1 shadow-2xs"
              >
            ...
            <div class="barcode-wrapper text-center mx-auto ms-md-auto me-md-0">
              <div class="barcode-box bg-white p-2 rounded-3 d-inline-block shadow-sm">
                <img
                  :src="getImageUrl('Barcode-Sticker-PNG-Transparent-Image 1.png')"
                  alt="الباركود"
                  class="barcode-img-large"
                />
              </div>
              <button
        ...
        <div class="row g-3 g-lg-4 mb-3 mb-md-4">
          <div class="col-12 col-lg-6">
            <div
              class="card border-0 rounded-4 p-3 p-md-4 bg-white shadow-sm h-100 d-flex flex-column justify-content-between"
            >
              <div>
                <h5
                  class="fw-bold text-dark mb-3 mb-md-4 fs-6 fs-md-5 d-flex align-items-center justify-content-between"
                  :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                >
                  <span>{{ t("humanImpactTitle") }}</span>
                  <span
                    class="badge bg-danger-subtle text-danger fs-10 rounded-pill px-2 py-1 fw-bold"
                    ><i class="bi bi-robot me-1"></i>AI Impact Analytics</span
                  >
                </h5>

                <div class="row align-items-center text-center gy-3">
                  <!-- وحدات الدم المتبرع بها وبجانبها صورة القطرة والوعاء جهة اليسار -->
                  <div
                    class="col-6"
                    :class="currentLanguage === 'ar' ? 'border-start' : 'border-end'"
                  >
                    <div
                      class="d-flex align-items-center justify-content-center gap-2 gap-md-3 mb-1"
                    >
                      <h2 class="fw-black text-danger mb-0 fs-3 fs-md-2">
                        {{ displayUnitsDonated }}
                      </h2>
                      <img
                        :src="getIconUrl('22 (2).png')"
                        alt="وحدات الدم المتبرع بها"
                        class="impact-icon-img-large"
                        @error="handleImageFallback"
                      />
                    </div>
                    <small class="text-muted fs-8 d-block fw-bold mt-1">{{
                      t("unitsDonated")
                    }}</small>
                  </div>

                  <!-- حالة تم دعمها وبجانبها صورة الأيادي والقلب جهة اليسار -->
                  <div class="col-6">
                    <div
                      class="d-flex align-items-center justify-content-center gap-2 gap-md-3 mb-1"
                    >
                      <h2 class="fw-black text-danger mb-0 fs-3 fs-md-2">
                        {{ displayCasesSupported }}
                      </h2>
                      <img
                        :src="getImageUrl('hands.png')"
                        alt="حالات تم دعمها"
                        class="impact-icon-img-large"
                        @error="handleImageFallback"
                      />
              ...
              <div
                class="p-2.5 p-md-3 bg-pink-light rounded-3 text-center mt-3 mt-md-4 border-0 d-flex align-items-center justify-content-center gap-2"
              >
                <i class="bi bi-stars text-danger fs-6 flex-shrink-0"></i>
                <span class="text-danger fw-bold fs-8">{{
                  cardData?.ai_impact_statement || t("lifeImpactMsg")
                }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="col-12 col-lg-6">
            ...
            >
              <div>
                <h5
                  class="fw-bold text-dark mb-3 mb-md-4 fs-6 fs-md-5"
                  :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                >
                ...
                <div
                  class="d-flex align-items-center justify-content-between gap-3 mb-3 mb-md-4 flex-wrap flex-sm-nowrap"
                >
                  <div
                    class="flex-grow-1 min-w-0 w-100"
                    :class="currentLanguage === 'ar' ? 'text-start' : 'text-start'"
                  >
                    <div
                      class="d-flex align-items-baseline justify-content-start gap-2 mb-1"
                    >
                      <h2 class="fw-black text-danger mb-0 fs-2 fs-md-1">
                        {{ userPoints }}
                      </h2>
                    ...
                    <div
                      class="d-flex justify-content-between align-items-center fs-9 text-muted fw-bold"
                    >
                      <span class="text-truncate"
                        >{{ cardData?.points_needed || 150 }}
                        {{ t("pointsNeededText") }}</span
                      >
                  ...
                  <img
                    :src="getImageUrl('coins.png')"
                    alt="النقاط والمكافآت"
                    class="coins-img-large mx-auto mx-sm-0"
                    @error="handleImageFallback"
                  />
                </div>
              </div>
        ...
        <!-- 3. شارات الإنجاز الحقيقية من قاعدة البيانات والمحسوبة ديناميكياً -->
        <AchievementBadges
          :badges="badgesList"
          :donationsCount="activeDonationHistory.length"
          class="mb-3 mb-md-4"
        />
        ...
        <div class="card border-0 rounded-4 p-3 p-md-4 bg-white shadow-sm">
          <h5
            class="fw-bold text-dark mb-3 mb-md-4 text-center position-relative d-inline-block mx-auto section-title-line fs-6 fs-md-5"
          >
          ...
          <div class="table-responsive">
            <table
              class="table table-borderless align-middle mb-0 custom-history-table min-w-table"
              :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
            >
              <thead>
              ...
              <tbody class="fs-8">
                <tr
                  v-for="(item, index) in activeDonationHistory"
                  :key="item.id || index"
                  class="border-bottom text-nowrap"
                >
                  <td
                    class="text-dark fw-bold pe-3"
                    :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                  >
                  ...
                  <td class="text-center">
                    <span
                      :class="[
                        'badge px-3 px-md-4 py-2 rounded-3 fs-9 fw-bold',
                        item?.status === 'عاجلة' || item?.status === 'urgent'
                  ...
                  <td class="fw-bold text-secondary text-center pe-4">
                    +{{ item?.points_earned || 50 }}
                  </td>
                </tr>
                <tr v-if="!activeDonationHistory.length">
      ...
      <div
        ...
        class="modal-backdrop-custom d-flex align-items-center justify-content-center p-2 p-md-3"
      >
        <div
          class="card border-0 rounded-4 shadow-lg bg-white overflow-hidden modal-report-box w-100 max-w-750 position-relative"
          :class="currentLanguage === 'ar' ? 'dir-rtl' : 'dir-ltr'"
        >
          <!-- الهيدر الخاص بالتقرير -->
          <div
            class="bg-danger text-white p-3 p-md-4 d-flex align-items-center justify-content-between"
          >
          ...
          <div
            class="p-3 p-md-4 modal-scroll-body"
            style="max-height: 75vh; overflow-y: auto"
          >
            <!-- بطاقة تعريف المتبرع -->
            <div
              class="bg-light rounded-4 p-3 mb-3 border d-flex align-items-center justify-content-between flex-wrap gap-3"
            >
              <div class="d-flex align-items-center gap-3">
                ...
                <div :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'">
                  ...
                  <span
                    class="badge bg-danger-subtle text-danger fs-9 px-2 py-1 rounded-pill"
                    >{{ t("donorCode") }}:
                    {{ cardData?.donor_code || "BD123456789" }}</span
                  >
                </div>
              </div>
              <div :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'">
                <small class="text-muted fs-9 d-block">{{ t("bloodType") }}</small>
                <span class="fw-black text-danger fs-5" dir="ltr">{{
                  donorBloodType
                }}</span>
              </div>
            </div>
            ...
            <!-- المؤشرات -->
            <div
              class="alert alert-success border-0 bg-success-subtle text-success rounded-3 p-3 mb-3 d-flex align-items-center gap-3"
            >
              ...
              <div :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'">
                ...
                <p class="mb-0 fs-9 text-dark">
                  ...
                  <strong class="text-danger"
                    >{{ displayCasesSupported }} {{ t("modalHeroMsgPart2") }}</strong
                  >
                  {{ t("modalHeroMsgPart3") }}
                  <strong class="text-danger"
                    >{{ displayUnitsDonated }} {{ t("modalHeroMsgPart4") }}</strong
                  >. {{ t("modalHeroMsgPart5") }}
                </p>
              </div>
            </div>
            ...
            <div class="mb-3">
              <h6
                class="fw-bold text-dark border-bottom pb-2 mb-3 fs-8 d-flex align-items-center gap-2"
                :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
              >
              ...
              <div class="table-responsive">
                <table
                  class="table table-sm align-middle fs-9 mb-0"
                  :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                >
                  ...
                  <tbody>
                    <tr
                      v-for="(item, idx) in activeDonationHistory"
                      :key="idx"
                      class="border-bottom"
                    >
                      <td
                        class="fw-bold text-dark py-2 pe-3"
                        :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                      >
                      ...
                      >
                        {{
                          translateHospital(item?.hospital_name || item?.facility_name)
                        }}
                      </td>
                      <td class="text-center fw-bold text-dark py-2">
                        {{ item?.units || item?.units_donated || 1 }}
                      </td>
                      <td class="text-center text-success fw-bold py-2">
                        +{{ item?.points_earned || 50 }}
                      </td>
                    </tr>
                  </tbody>
          ...
          <!-- الفوتر -->
          <div
            class="p-3 bg-light border-top d-flex align-items-center justify-content-between flex-wrap gap-2"
          >
            <button
              type="button"
              class="btn btn-outline-secondary btn-sm px-3 rounded-pill fw-bold fs-9"
              @click="showReportModal = false"
            >
              {{ t("closeBtn") }}
            </button>
            <button
              type="button"
              class="btn btn-danger btn-sm px-4 rounded-pill fw-bold fs-9 d-flex align-items-center gap-1 shadow-sm"
              @click="printOrDownloadReport"
            >
      ...
      >
        ...
        >
          <div
            class="bg-dark text-white p-3 p-md-4 d-flex align-items-center justify-content-between"
          >
            <div class="d-flex align-items-center gap-2">
          ...
          <div class="p-3 p-md-4">
            <div
              class="alert alert-warning border-0 bg-warning-subtle text-dark rounded-3 p-3 mb-3 d-flex align-items-center justify-content-between"
            >
              <span class="fw-bold fs-8">{{ t("yourCurrentBalance") }}:</span>
              <span class="fs-5 fw-black text-danger"
                >{{ userPoints }} {{ t("pointsText") }}</span
              >
            </div>
            ...
            <div class="row g-3">
              <div v-for="reward in rewardsList" :key="reward.id" class="col-12 col-md-6">
                <div
                  class="p-3 border rounded-4 bg-white h-100 d-flex flex-column justify-content-between shadow-2xs"
                >
                  <div>
                    <div class="d-flex align-items-center justify-content-between mb-2">
                      <span class="badge bg-danger-subtle text-danger fw-bold fs-9">{{
                        translateRewardCategory(reward.category)
                      }}</span>
                      <strong class="text-warning fs-8"
                        ><i class="bi bi-coin me-1"></i>{{ reward.cost }}
                        {{ t("pointsText") }}</strong
                      >
                    </div>
                    <h6
                      class="fw-bold text-dark fs-7 mb-1"
                      :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                    >
                      {{ translateRewardTitle(reward) }}
                    </h6>
                    <p
                      class="text-muted fs-9 mb-2"
                      :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
                    >
                      {{ translateRewardDesc(reward) }}
                    </p>
                  </div>
                  <button
          ...
          <div class="p-3 bg-light border-top text-end">
            <button
              type="button"
              class="btn btn-secondary btn-sm px-4 rounded-pill fw-bold fs-9"
              @click="showRedeemModal = false"
            >
              {{ t("closeBtn") }}
            </button>
...
const baseUnits = ref(0);
const baseCases = ref(0);
...
const translateLevel = (lvl) => {
  if (!lvl)
    return currentLanguage.value === "en"
      ? translations.en.levelBeginner
      : translations.ar.levelBeginner;
  if (currentLanguage.value === "en") {
    if (lvl === "متقدم" || lvl === "Advanced") return translations.en.levelAdvanced;
    if (lvl === "متبرع مبتدئ" || lvl === "Beginner Donor")
      return translations.en.levelBeginner;
  }
  return lvl;
};
...
const displayUnitsDonated = computed(() => {
  let localUnits = 0;
  const localAccepted = JSON.parse(
    localStorage.getItem("musaef_accepted_donations") || "[]"
  );
  localAccepted.forEach((item) => {
    localUnits += Number(item.units) || 1;
  });
  return (baseUnits.value || 0) + localUnits;
});

const displayCasesSupported = computed(() => {
  const localAccepted = JSON.parse(
    localStorage.getItem("musaef_accepted_donations") || "[]"
  );
  return (baseCases.value || 0) + localAccepted.length;
});

const donorName = computed(() => {
  return (
    authStore.userName || authStore.user?.name || cardData.value?.donor_name || "متبرع"
  );
});

const donorBloodType = computed(() => {
  return (
    authStore.user?.blood_type_name ||
    authStore.user?.donor?.blood_type?.name ||
    cardData.value?.blood_type ||
    "O+"
  );
});
...
// ربط صورة المتبرع ديناميكياً مع المتجر والذاكرة المحلية
...
const applyLocalDonationUpdates = () => {
  const localAccepted = JSON.parse(
    localStorage.getItem("musaef_accepted_donations") || "[]"
  );

  const currentList = [...donationHistory.value];
  localAccepted.forEach((localItem) => {
    if (!currentList.some((item) => item.id === localItem.id)) {
      currentList.unshift(localItem);
    }
  });
  donationHistory.value = currentList;

  const extraPoints = localAccepted.length * 50;
  userPoints.value = basePoints.value + extraPoints;
};

const fetchAchievementsData = async (isBackground = false) => {
  ...
  try {
    ...
    if (payload.points !== undefined && (!basePoints.value || !isBackground)) {
      basePoints.value = payload.points;
    }
    if (payload.units_donated !== undefined && (!baseUnits.value || !isBackground)) {
      baseUnits.value = payload.units_donated;
    }
    if (payload.cases_supported !== undefined && (!baseCases.value || !isBackground)) {
      baseCases.value = payload.cases_supported;
    }
    ...
    if (Array.isArray(history)) {
      donationHistory.value = history.filter((item) => item !== null);
    }

    applyLocalDonationUpdates();
  } catch (error) {
    console.error("خطأ في جلب بيانات الإنجازات:", error);
  } finally {
    if (!isBackground) loading.value = false;
  }
};

const handleDonationAcceptedEvent = () => {
  applyLocalDonationUpdates();
  fetchAchievementsData(true);
};

const handleProfileUpdatedEvent = (e) => {
  if (e?.detail) {
    if (authStore.setUser) {
      authStore.setUser(e.detail);
    } else if (authStore.user) {
      Object.assign(authStore.user, e.detail);
    }
  }
  fetchAchievementsData(true);
};
...
onMounted(() => {
  ...
  window.addEventListener("musaef_donation_accepted", handleDonationAcceptedEvent);
  window.addEventListener("musaef_profile_updated", handleProfileUpdatedEvent);
...
onUnmounted(() => {
  ...
  window.removeEventListener("musaef_donation_accepted", handleDonationAcceptedEvent);
  window.removeEventListener("musaef_profile_updated", handleProfileUpdatedEvent);
});
