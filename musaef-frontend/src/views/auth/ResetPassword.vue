<template>
  <div class="auth-page-wrapper min-vh-100 d-flex align-items-center justify-content-center dir-rtl">
    <div class="container py-4">
      <div class="row g-0 auth-main-card shadow-lg rounded-5 overflow-hidden bg-white mx-auto position-relative">
        ...
        <div class="col-lg-6 form-section-col p-4 p-md-5 d-flex flex-column justify-content-center position-relative bg-light-gray">
          ...
          <div class="auth-card-inner bg-white rounded-4 p-4 shadow-sm mt-4 mx-auto w-100">

            <!-- التبويبات الرئيسية (تسجيل الدخول / إنشاء حساب) -->
            <div class="nav-tabs-header d-flex border-bottom mb-4 position-relative text-center">
              <button
                type="button"
            ...
            <!-- تنبيه الأخطاء إن وجدت من الـ Store -->
            <div v-if="authStore.error" class="alert alert-danger rounded-3 fs-8 mb-3 text-center">
              {{ authStore.error }}
            </div>
            ...
            <form v-if="activeTab === 'login'" @submit.prevent="handleLogin">
              ...
              <div class="d-flex justify-content-between align-items-center mb-4 fs-8">
                ...
                <RouterLink to="/forgot-password" class="text-danger text-decoration-none fw-bold fs-8">هل نسيت كلمة السر؟</RouterLink>
              </div>
              ...
              <!-- أزرار الدخول الاجتماعي التفاعلية -->
              <div class="d-flex gap-2 mb-3">
                <button
                  type="button"
                  class="btn btn-social flex-fill rounded-3 py-2 d-flex align-items-center justify-content-center gap-2 border"
            ...
            <form v-else @submit.prevent="handleRegister">
              ...
              <div class="mb-2 text-end">
                <div class="input-group custom-input-group">
                  ...
                  <input v-model="registerForm.password" type="password" class="form-control border-end-0 text-end fs-8 order-1" placeholder="كلمة المرور" required />
                </div>
              </div>
                ...
                <div class="row g-2">
                  <div class="col-6">
                    ...
                    >
                      ...
                      <span class="fs-8 fw-bold">مستشفى / بنك دم</span>
                      <i class="bi bi-building fs-6 ms-1"></i>
                    </div>
                  </div>
                  <div class="col-6">
                    <div
                    ...
                    >
                      ...
                      <i class="bi bi-person-fill fs-6 ms-1"></i>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div class="p-3 bg-light rounded-3 border mb-3">
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      <select v-model="registerForm.bloodType" class="form-select border-end-0 text-end fs-8 order-1">
                        <option value="" disabled selected>اختر فصيلة الدم</option>
                        <option v-for="type in ['+A', '-A', '+B', '-B', '+AB', '-AB', '+O', '-O']" :key="type" :value="type">{{ type }}</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      <input v-model="registerForm.birthDate" type="date" class="form-control border-end-0 text-end fs-8 order-1" />
                    </div>
                  </div>
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      <select v-model="registerForm.gender" class="form-select border-end-0 text-end fs-8 order-1">
                ...
                <div class="p-3 bg-light rounded-3 border mb-3">
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      <input v-model="registerForm.hospitalName" type="text" class="form-control border-end-0 text-end fs-8 order-1" placeholder="اسم الجهة" />
                    </div>
                  </div>
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      <input v-model="registerForm.licenseNumber" type="text" class="form-control border-end-0 text-end fs-8 order-1" placeholder="Medical License Number" />
                    </div>
                  </div>
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="input-group custom-input-group">
                      <span class="input-group-text bg-white text-muted border-start-0 order-2"><i class="bi bi-person"></i></span>
                      <input v-model="registerForm.managerName" type="text" class="form-control border-end-0 text-end fs-8 order-1" placeholder="الاسم الأول" />
                    </div>
                  </div>
                  ...
                  <div class="mb-2 text-end">
                    ...
                    <div class="upload-dashed-box border border-dashed rounded-3 p-3 text-center bg-white cursor-pointer" @click="triggerFileInput">
                      ...
                      <small class="text-muted d-block fs-8">اسحب الملف هنا او اختر ملف</small>
                      <small class="text-muted d-block fs-9">PDF / Image(SMB)</small>
                      <input type="file" ref="fileInput" @change="handleFileUpload" class="d-none" id="licenseFile" />
                    </div>
                  </div>
                </div>
              </template>
        ...
        <!-- ================= 2. القسم الأيسر: البانر البصري والعبارة بالمنتصف ================= -->
        <div class="col-lg-6 hero-visual-col position-relative d-flex flex-column justify-content-start p-4 p-md-5">
          <img
          ...
          <div class="hero-content position-relative z-2 text-center pt-3 mx-auto">
            ...
            <p class="hero-description text-dark fw-bold mb-4 text-center">
              نوصلك بالمتبرع المناسب في أسرع وقت لإنقاذ حياة محتاجة.
            </p>

            <div class="row g-2 justify-content-center text-center mt-4">
              <div class="col-3">
...
const handleLogin = async () => {
  ...
  try {
    if (authStore.login) {
      ...
      authStore.user = { name: 'مستخدم تجريبي', email: loginForm.value.email };
      authStore.role = 'admin';
    }

    if (authStore.userRole === 'admin' || authStore.role === 'admin') {
...
const handleSocialLogin = async (provider) => {
  loading.value = true;
  try {
    if (authStore.loginWithSocial) {
      await authStore.loginWithSocial(provider);
    } else {
      authStore.user = { name: `مستخدم ${provider}`, email: `${provider}@musaef.com` };
    }
    router.push('/');
  } catch (err) {
    console.error(err);
...
const handleRegister = async () => {
  ...
  try {
    ...
    if (authStore.register) {
      ...
      authStore.user = { name: registerForm.value.fullName, email: registerForm.value.email };
    }

    if (accountType.value === 'hospital') {
      alert('تم إرسال طلب تسجيل المستشفى بنجاح، سيتم مراجعته والتواصل معكم قريباً.');
