<template>
  ...
  >
    <div class="container px-2 px-md-3">
      <div
        class="row g-0 auth-main-card shadow-lg rounded-5 overflow-hidden bg-white mx-auto position-relative"
      >
        <!-- ================= 1. قسم النماذج والمدخلات ================= -->
        <div
          class="col-12 col-lg-6 form-section-col p-3 p-sm-4 p-md-5 d-flex flex-column justify-content-center position-relative bg-light-gray order-2 order-lg-1"
        >
          <!-- زر تحويل اللغة والشعار أعلى المربع -->
          <div
            class="top-logo-container position-absolute top-0 p-3 p-md-4 z-3 d-flex align-items-center gap-2"
            :class="currentLanguage === 'ar' ? 'end-0' : 'start-0'"
          >
            <img
              :src="getImageUrl('auth-logo.png')"
              alt="مسعف"
              class="top-auth-logo"
              @error="handleLogoFallback"
            />

            <button
              class="btn btn-sm btn-light rounded-pill border fs-9 fw-bold px-2 py-1 ms-2"
          ...
          <div
            class="auth-card-inner bg-white rounded-4 p-3 p-sm-4 shadow-sm mt-5 mt-lg-4 mx-auto w-100"
          >
            <!-- التبويبات الرئيسية (تسجيل الدخول / إنشاء حساب) -->
            <div
              class="nav-tabs-header d-flex border-bottom mb-3 mb-md-4 position-relative text-center"
            >
              <button
                type="button"
            ...
            <div
              ...
              class="alert alert-danger rounded-3 fs-8 mb-3 text-center"
            >
            ...
            <div
              v-if="successMessage"
              class="alert alert-success rounded-3 fs-8 mb-3 text-center"
            >
              {{ successMessage }}
            </div>
            ...
            <form v-if="activeTab === 'login'" @submit.prevent="onLogin">
              ...
              >
                <label class="form-label fs-8 text-dark fw-bold mb-1 d-block">{{
                  t("email")
                }}</label>
                <div class="input-group custom-input-group">
                  <span
                    ...
                    :class="
                      currentLanguage === 'ar'
                        ? 'border-start-0 order-2'
                        : 'border-end-0 order-1'
                    "
                  >
                  ...
                  <input
                    ...
                    :class="
                      currentLanguage === 'ar'
                        ? 'border-end-0 text-end order-1'
                        : 'border-start-0 text-start order-2'
                    "
              ...
              <div
                class="d-flex justify-content-between align-items-center mb-4 fs-8 flex-wrap gap-2"
              >
                <div class="form-check d-flex align-items-center gap-2 p-0">
                  <input
                    v-model="loginForm.remember"
                    type="checkbox"
                    class="form-check-input ms-0 custom-checkbox"
                    id="rememberMe"
                  />
                  <label
                ...
                <router-link
                  to="/forgot-password"
                  class="text-danger text-decoration-none fw-bold fs-8"
              ...
              <button
                type="submit"
                class="btn btn-danger w-100 rounded-3 py-2 fw-bold text-white shadow-sm mb-4 btn-submit-red"
                :disabled="loading"
              >
                <span
                  v-if="loading"
              ...
              <div class="text-center position-relative mb-4">
                ...
                <span
                  class="position-absolute top-50 start-50 translate-middle bg-white px-3 text-muted fs-8"
                  >{{ t("orSocial") }}</span
                >
              </div>

              <!-- أزرار الدخول الاجتماعي المفعلة -->
              <div class="d-flex gap-2 mb-3 flex-wrap">
                <button
                  type="button"
                  class="btn btn-social flex-fill rounded-3 py-2 d-flex align-items-center justify-content-center gap-1.5 border"
            ...
            <form v-else @submit.prevent="onRegister">
              ...
              <div
                ...
                :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
              >
                <label class="form-label fs-8 text-dark fw-bold mb-2 d-block">{{
                  t("accountTypeLabel")
                }}</label>
                <div class="row g-2">
                  <div class="col-6">
                    <div
                      ...
                      :class="
                        accountType === 'hospital'
                          ? 'border-danger bg-pink-light text-danger'
                          : 'text-muted bg-white'
                      "
                    ...
                    >
                      <input
                        ...
                        class="form-check-input ms-0 custom-radio"
                      />
                      <span class="fs-8 fw-bold text-truncate">{{
                        t("hospitalAccount")
                      }}</span>
              ...
              <template v-if="accountType === 'donor'">
                <div class="p-3 bg-light rounded-3 border mb-3">
                  <h6 class="text-danger fw-bold text-center mb-3 fs-8">
                    {{ t("donorExtraInfoTitle") }}
                  </h6>
                ...
                <div class="p-3 bg-light rounded-3 border mb-3">
                  ...
                  >
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      ></span>
                      <input
                        v-model="registerForm.facilityName"
                        type="text"
                        class="form-control fs-8"
                        :class="
                          currentLanguage === 'ar'
                  ...
                  >
                    ...
                    <div class="row g-2">
                      <div class="col-6">
                        <div
                          class="border rounded-3 p-2 text-center fs-8 bg-white d-flex align-items-center justify-content-between"
                        >
                          <input
                            type="radio"
                            v-model="registerForm.facilityType"
                            value="blood_bank"
                            class="form-check-input ms-0 custom-radio"
                          />
                  ...
                  >
                    ...
                    <div class="input-group custom-input-group">
                      ...
                      ></span>
                      <input
                        v-model="registerForm.managerName"
                        type="text"
                        class="form-control fs-8"
                        :class="
                          currentLanguage === 'ar'
                  ...
                  >
                    ...
                    <div
                      class="upload-dashed-box border border-dashed rounded-3 p-3 text-center bg-white cursor-pointer"
                      @click="triggerFileInput"
                    >
                      <i
                        class="bi bi-file-earmark-arrow-up fs-3 text-muted d-block mb-1"
                      ></i>
                      ...
                      <small class="text-muted d-block fs-9">PDF / Image (Max 5MB)</small>
                      <input
                        type="file"
                        ref="fileInput"
              ...
              <!-- الموافقة على الشروط -->
              <div
                class="form-check mb-3 fs-8 d-flex align-items-center justify-content-center gap-1"
                :class="currentLanguage === 'ar' ? 'text-end' : 'text-start'"
              >
                <input
              ...
              <button
                type="submit"
                class="btn btn-danger w-100 rounded-3 py-2 fw-bold text-white shadow-sm mb-2 btn-submit-red"
                :disabled="loading"
              >
                <span v-if="loading" class="spinner-border spinner-border-sm me-2"></span>
                {{
                  accountType === "hospital"
                    ? t("submitHospitalRegister")
                    : t("createAccountBtn")
                }}
              </button>
        ...
        <div
          class="col-12 col-lg-6 hero-visual-col position-relative d-flex flex-column justify-content-start p-4 p-md-5 order-1 order-lg-2"
        >
          ...
          <div class="hero-content position-relative z-2 text-center pt-3 mx-auto">
            <h2 class="fw-bold hero-heading text-dark mb-1 text-center">
              {{ t("heroHeadingPart1") }}
              <span class="text-danger">{{ t("heroHeadingPart2") }}</span>
            </h2>

            <!-- رسمة نبض القلب -->
            <div class="pulse-line-wrapper my-2 mx-auto text-center">
              <img
                :src="getIconUrl('Vector 7.png')"
                alt="نبض القلب"
                class="pulse-vector-img mx-auto"
              />
            </div>
            ...
            <div class="row g-2 justify-content-center text-center mt-3 mt-md-4">
              <div class="col-3">
                <div class="feature-icon-box mx-auto mb-2">
                  <img
                    :src="getIconUrl('Frame 2147225319.png')"
                    alt="إنقاذ الأرواح"
                    class="feature-img"
                  />
                </div>
                <span class="d-block feature-label text-dark fw-bold">{{
                  t("featureSavingLives")
                }}</span>
              </div>
              ...
              <div class="col-3">
                <div class="feature-icon-box mx-auto mb-2">
                  <img
                    :src="getIconUrl('Frame 2147225318.png')"
                    alt="استجابة سريعة"
                    class="feature-img"
                  />
                </div>
              ...
              <div class="col-3">
                <div class="feature-icon-box mx-auto mb-2">
                  <img
                    :src="getIconUrl('Frame 2147225317.png')"
                    alt="مجتمع المتبرعين"
                    class="feature-img"
                  />
                </div>
              ...
              <div class="col-3">
                <div class="feature-icon-box mx-auto mb-2">
                  <img
                    :src="getIconUrl('Frame 2147225316.png')"
                    alt="آمن وموثوق"
                    class="feature-img"
                  />
                </div>
...
const translations = {
  ar: {
    ...
    hospitalSuccessMsg:
      "تم إرسال طلب تسجيل الجهة الطبية بنجاح. سيتم مراجعته والتواصل معكم قريباً.",
  },
  en: {
    loginTab: "Login",
    ...
    hospitalSuccessMsg:
      "Medical facility registration request submitted successfully. We will review and contact you soon.",
  },
};
...
const notifyDonorActivity = () => {
  window.dispatchEvent(
    new CustomEvent("donor-activity-updated", {
...
const onLogin = async () => {
  ...
  const user = authStore.user || (result && result.user);
...
const onRegister = async () => {
  ...
  if (
    accountType.value === "donor" &&
    result &&
    (result.success || result.user || result.token)
  ) {
    notifyDonorActivity();
  }
  if (result && result.success && accountType.value === "hospital") {
...
/**
 * -------------------------------------------------------------
 * تفعيل الدخول عبر الشبكات الاجتماعية (Google, Facebook, Apple)
 * -------------------------------------------------------------
 */
const handleSocialLogin = (provider) => {
  const backendBaseUrl = import.meta.env.VITE_API_BASE_URL || "http://localhost:8000/api";

  // إعادة التوجيه إلى مسار لاراڤيل المعني بخدمة Socialite
  window.location.href = `${backendBaseUrl}/auth/social/${provider}/redirect`;
};
