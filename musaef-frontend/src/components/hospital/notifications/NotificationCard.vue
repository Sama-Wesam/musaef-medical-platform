<template>
  <div
    class="card border-0 shadow-sm p-3 p-md-4 rounded-4 notification-card position-relative"
    :class="!item.read && !item.unread ? 'bg-unread-custom' : 'bg-white'"
  >
    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between gap-3">

      <!-- أقصى اليمين: الأيقونة الدائرية الملونة + العنوان والوصف -->
      <div class="d-flex align-items-start align-items-sm-center gap-3 text-end flex-grow-1 w-100">
      ...
      <!-- أقصى اليسار: الوقت والشارة + زر القائمة -->
      <div class="d-flex align-items-center justify-content-between justify-content-sm-end w-100 w-sm-auto gap-3 pt-2 pt-sm-0 border-top border-top-sm-0 border-light-subtle">
        <div class="d-flex align-items-center gap-2 text-muted fs-9 fs-md-8">
          ...
          <span v-if="!item.read && !item.unread" class="status-dot rounded-circle bg-danger" title="غير مقروء"></span>

          <span v-else class="badge read-badge rounded-pill fw-medium px-2 py-1">مقروء</span>
        </div>
        <button class="btn btn-link text-muted p-0 text-decoration-none fs-5 dot-menu-btn me-sm-1" aria-label="خيارات">⋮</button>
      </div>
