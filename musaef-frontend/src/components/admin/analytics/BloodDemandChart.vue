<template>
  <div class="card border-0 shadow-sm p-3 p-md-4 rounded-4 bg-white mb-3 mb-md-4" :dir="langStore.dir">
    <div class="d-flex align-items-center justify-content-between mb-3 mb-md-4 flex-wrap gap-2">
      ...
      <span class="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill fs-9 px-2.5 py-1">
        {{ t('aiAnalyticBadge') }}
      </span>
    </div>
    ...
    <div class="chart-scroll-wrapper overflow-x-auto">
      <div class="position-relative pt-3 pb-1 min-chart-width" style="height: 240px; direction: ltr;">
        ...
        <div class="chart-y-axis position-absolute h-100 start-0 w-100 d-flex flex-column justify-content-between text-secondary fs-8 fw-semibold" style="pointer-events: none;">
          <div v-for="scale in [5000, 4000, 3000, 2000, 1000, 0]" :key="scale" class="d-flex align-items-center w-100">
            <span class="pe-2 text-end" style="width: 45px;">{{ scale }}</span>
            <div class="border-bottom flex-grow-1 border-secondary-subtle" :style="{ borderStyle: scale === 0 ? 'solid !important' : 'dotted !important' }"></div>
          </div>
        </div>

        <!-- أعمدة المخطط البياني الملونة والتفاعلية -->
        <div class="d-flex align-items-end justify-content-around h-100 position-relative z-1" style="padding-left: 50px; padding-right: 20px;">
          ...
          >
            ...
            <div
              class="bar-column w-100 position-relative"
              :style="{ height: b.calculatedHeight + 'px', backgroundColor: b.color, borderRadius: '6px 6px 0 0' }"
              :title="`${b.type}: ${b.count}`"
            >
              <!-- تأثير إشراق تفاعلي عند Hover -->
...
// حساب أطوال الأعمدة بنسبة وتناسب مع أقصى ارتفاع متاح (180px)
const normalizedDemand = computed(() => {
  ...
  return (props.bloodDemand || []).map((item) => {
    ...
    const height = Math.min(Math.max((val / maxScale) * maxHeightPx, 12), maxHeightPx);
    return {
      ...item,
