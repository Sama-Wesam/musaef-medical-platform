<template>
  <header class="hospital-header" :dir="currentLocale === 'ar' ? 'rtl' : 'ltr'">
        ...
        <div class="doctor-info">
          ...
          <div class="doctor-text d-none d-sm-flex" :class="currentLocale === 'ar' ? 'text-end' : 'text-start'">
            <h6 class="doctor-name">
              {{ user?.name || (currentLocale === 'en' ? 'Super Admin' : 'مدير النظام - Super Admin') }}
            </h6>
            <span class="doctor-role">
              {{ userRole === 'admin' ? (currentLocale === 'en' ? 'System Admin' : 'مدير النظام') : (currentLocale === 'en' ? 'Blood Bank Director' : 'مدير بنك الدم') }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="header-center">
        ...
        <div class="search-box d-none d-md-flex">
          <input
            type="text"
            v-model="search"
            :placeholder="currentLocale === 'en' ? 'Search for patient or request...' : 'ابحث عن مريض أو طلب'"
            class="search-input"
            :class="currentLocale === 'ar' ? 'text-end' : 'text-start'"
            @input="handleSearch"
          />
          <img :src="searchIcon" alt="Search" class="search-icon" :style="currentLocale === 'en' ? 'left: 14px; right: auto;' : 'right: 14px; left: auto;'" />
        </div>
        ...
        <div class="dropdown position-relative">
          ...
          <ul class="dropdown-menu shadow-sm border-0 rounded-4 p-3 mt-2 fs-8 notifications-dropdown-menu" :class="currentLocale === 'ar' ? 'dropdown-menu-end text-end' : 'dropdown-menu-start text-start'" aria-labelledby="adminNotificationsDropdown">
            <li class="d-flex align-items-center justify-content-between mb-2 pb-2 border-bottom">
              <span class="fw-bold text-dark">{{ currentLocale === 'en' ? 'AI System Notifications' : 'إشعارات النظام الذكية (AI)' }}</span>
              <span class="badge bg-danger-subtle text-danger rounded-pill px-2 py-0.5 fs-9">{{ unreadCount }} {{ currentLocale === 'en' ? 'new' : 'جديدة' }}</span>
            </li>

            <div style="max-height: 240px; overflow-y: auto;">
              <li v-for="notif in translatedNotifications" :key="notif.id" class="py-2 border-bottom cursor-pointer notification-item" @click="handleNotificationClick(notif)">
              ...
              <li v-if="translatedNotifications.length === 0" class="text-center text-muted py-3 fs-9">
                {{ currentLocale === 'en' ? 'No new notifications.' : 'لا توجد إشعارات جديدة.' }}
              </li>
            </div>

            <li class="pt-2 text-center border-top mt-2">
              <button @click.prevent="handleViewAllSystemNotifications" class="btn btn-link text-danger fw-bold fs-9 text-decoration-none p-0 border-0 w-100 text-center cursor-pointer">
                {{ currentLocale === 'en' ? 'View All System Notifications >' : 'عرض جميع إشعارات النظام >' }}
              </button>
            </li>
          </ul>
        </div>

        <!-- زر تبديل اللغة المضاف بجانب زر "منخفض O-" -->
        <div class="dropdown">
          <button
          ...
          <ul class="dropdown-menu shadow-sm rounded-3 border-0 mt-1" :class="currentLocale === 'ar' ? 'dropdown-menu-end' : 'dropdown-menu-start'" aria-labelledby="adminLanguageDropdown">
            <li>
        ...
        <div class="blood-alert d-none d-sm-flex cursor-pointer shadow-2xs" @click="handleBloodAlertClick" :title="currentLocale === 'en' ? 'Click for AI Forecast' : 'انقر لعرض تفاصيل تحليل النقص الذكي'">
    ...
    <div v-if="showNotificationsModal" class="modal-backdrop-custom d-flex align-items-center justify-content-center">
      <div class="modal-card bg-white rounded-4 shadow-lg p-4 position-relative" :class="currentLocale === 'ar' ? 'text-end' : 'text-start'" style="width: 90%; max-width: 500px; z-index: 1050;">
        <button type="button" class="btn-close position-absolute top-0 m-3" :class="currentLocale === 'ar' ? 'start-0' : 'end-0'" @click="showNotificationsModal = false" aria-label="Close"></button>

        <div class="d-flex align-items-center gap-2 text-danger mb-3 border-bottom pb-2">
          <i class="bi bi-bell-fill fs-4"></i>
          <h5 class="fw-bold mb-0">
            {{ currentLocale === 'en' ? 'Interactive AI System Notifications' : 'إشعارات النظام الذكية التفاعلية' }}
          </h5>
        </div>
        ...
        <div v-if="isLoadingModal" class="text-center py-4 text-muted">
          ...
          <p class="mb-0 fw-semibold">
            {{ currentLocale === 'en' ? 'Opening all system notifications interactively...' : 'جاري فتح جميع إشعارات النظام بشكل تفاعلي...' }}
          </p>
        </div>

        <div v-else>
          <div class="alert alert-danger-subtle text-danger border-0 rounded-3 py-2 px-3 mb-3 fs-8 fw-semibold text-center">
            🔔 {{ currentLocale === 'en' ? 'Opening all system notifications interactively' : 'جاري فتح جميع إشعارات النظام بشكل تفاعلي' }}
          </div>

          <div style="max-height: 320px; overflow-y: auto;" class="pe-1">
            <div v-for="notif in translatedNotifications" :key="notif.id" class="p-3 mb-2 rounded-3 bg-light border border-light-subtle">
              <div class="d-flex align-items-center justify-content-between mb-1">
                <span class="fw-bold text-danger fs-8">{{ notif.title }}</span>
                <span class="text-muted fs-10">{{ notif.time }}</span>
              </div>
              <p class="text-dark fs-8 mb-0">{{ notif.message }}</p>
            </div>
            <div v-if="translatedNotifications.length === 0" class="text-center text-muted py-3 fs-8">
              {{ currentLocale === 'en' ? 'No notifications recorded.' : 'لا توجد إشعارات مسجلة.' }}
            </div>
          </div>

          <div class="text-center mt-3 pt-2 border-top">
            <button class="btn btn-secondary btn-sm rounded-pill px-4" @click="showNotificationsModal = false">
...
const rawNotificationsList = ref([
  {
    ...
    messageEn: 'O- blood group stock has dropped below safe limits in sector hospitals.',
    timeAr: 'منذ 5 دقائق',
    timeEn: '5 mins ago',
    read: false
  },
...
// قائمة الإشعارات المترجمة ديناميكياً بناءً على اختيار اللغة
const translatedNotifications = computed(() => {
  return rawNotificationsList.value.map(n => ({
    id: n.id,
    title: currentLocale.value === 'en' ? (n.titleEn || n.title) : (n.titleAr || n.title),
    message: currentLocale.value === 'en' ? (n.messageEn || n.message) : (n.messageAr || n.message),
    time: currentLocale.value === 'en' ? (n.timeEn || n.time) : (n.timeAr || n.time),
    read: n.read
  }));
});
...
const fetchLiveNotifications = async () => {
  try {
    ...
    console.warn('استخدام التنبيهات الافتراضية.');
  }
};
...
@media (min-width: 1200px) { .search-box { width: 360px; height: 46px; } }
...
@media (min-width: 992px) { .blood-alert { width: 140px; height: 42px; font-size: 16px; } }
...
/* تنسيق نافذة المودال التفاعلية للاشتعارات */
