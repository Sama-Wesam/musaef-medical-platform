      ...
      <div class="col-12 col-lg-7">
        <div class="card border-0 shadow-sm p-3 p-md-4 rounded-4 bg-white h-100">
          ...
          <div class="d-flex flex-column gap-3">
            <!-- مدخلات ملفات مخفية لرفع الشعار والأيقونة -->
            <input
              type="file"
              ref="logoFileInput"
              class="d-none"
            ...
            >
              <label class="fw-bold text-dark fs-8 text-nowrap general-label-width">{{
                t("platformName")
              }}</label>
            ...
            >
              ...
              <div
                class="upload-box flex-grow-1 d-flex align-items-center justify-content-between p-1 px-2 border rounded-3 bg-white w-100"
              >
                <div class="d-flex align-items-center gap-1 gap-sm-2">
                  <button
                    class="btn btn-outline-danger btn-sm rounded-2 px-2 px-sm-3 py-1 fs-9 d-flex align-items-center gap-1"
                    @click="deleteLogo"
                  >
                    <svg
                      width="14"
                  ...
                  <button
                    class="btn btn-light border btn-sm text-muted rounded-2 px-2 px-sm-3 py-1 fs-9 d-flex align-items-center gap-1"
                ...
                <div
                  class="logo-preview-box px-2 d-flex align-items-center justify-content-center"
                >
                  <img
                    v-if="logoPreview"
                    :src="logoPreview"
                  ...
                  />
                  <span v-else class="text-muted fs-9 fst-italic">{{
                    currentLanguage === "ar" ? "لا يوجد شعار" : "No Logo"
                  }}</span>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <div
                  class="favicon-preview-box px-2 d-flex align-items-center justify-content-center"
                >
                  <img
                    v-if="faviconPreview"
                    :src="faviconPreview"
                  ...
                  />
                  <span v-else class="text-muted fs-9 fst-italic">{{
                    currentLanguage === "ar" ? "لا توجد أيقونة" : "No Favicon"
                  }}</span>
                </div>
              </div>
            </div>
      ...
      <div class="col-12 col-lg-5">
        <div class="d-flex flex-column gap-3 gap-md-4">
          ...
          <div class="card border-0 shadow-sm p-3 p-md-4 rounded-4 bg-white">
            ...
            <div class="mb-2">
              <label class="fw-bold text-dark fs-8 d-block mb-2">{{
                t("timezoneLabel")
              }}</label>
          ...
          <div class="card border-0 shadow-sm p-3 p-md-4 rounded-4 bg-white">
            ...
            <div class="d-flex flex-column gap-3">
              <!-- 1. وضع الصيانة -->
              <div
                class="d-flex align-items-center justify-content-between p-2 border-bottom flex-wrap gap-2"
              >
                <div class="d-flex align-items-center gap-2 gap-sm-3 min-w-0">
                  <div
                    class="quick-icon-box bg-success-subtle text-success rounded-3 p-2 flex-shrink-0"
                  >
                    <svg
                      width="18"
                      height="18"
              ...
              >
                <div class="d-flex align-items-center gap-2 gap-sm-3 min-w-0">
                  <div
                    class="quick-icon-box bg-purple-subtle text-purple rounded-3 p-2 flex-shrink-0"
                  >
                    <svg
                      width="18"
                      height="18"
                ...
                >
                  <input
                    class="form-check-input custom-switch"
                    type="checkbox"
                    v-model="generalSettings.selfRegistration"
                  />
                </div>
              </div>
              ...
              >
                <div class="d-flex align-items-center gap-2 gap-sm-3 min-w-0">
                  <div
                    class="quick-icon-box bg-primary-subtle text-primary rounded-3 p-2 flex-shrink-0"
                  >
                    <svg
                      width="18"
                      height="18"
                ...
                >
                  <input
                    class="form-check-input custom-switch"
                    type="checkbox"
                    v-model="generalSettings.twoFactorAuth"
                  />
                </div>
              </div>
            </div>
...
// دالة لتحديث الأيقونة في أعلى تبويب المتصفح مباشرة
const applyFaviconToDocument = (url) => {
  if (!url) return;
...
// المراجع لمدخلات الملفات المخفية
const logoFileInput = ref(null);
const faviconFileInput = ref(null);

// صور المعاينة الافتراضية للـ Logo والـ Favicon
const getImageUrl = (fileName) => {
  return new URL(`../../../assets/images/${fileName}`, import.meta.url).href;
};

const logoPreview = ref(
  props.generalSettings.logoUrl ||
    localStorage.getItem("musaef_logo_url") ||
    getImageUrl("logo.png")
);

const savedFavicon =
  props.generalSettings.faviconUrl || localStorage.getItem("musaef_favicon_url");
const faviconPreview = ref(savedFavicon || getImageUrl("auth-logo.png"));

onMounted(() => {
  window.addEventListener("storage", syncLanguage);
  window.addEventListener("musaef_lang_changed", syncLanguage);

  // التأكد من تطبيق Favicon دائماً عند تحميل المكون
  if (faviconPreview.value) {
    applyFaviconToDocument(faviconPreview.value);
  }
});
...
const dictionary = {
  ar: {
    ...
    "2faDesc": "فرض المصادقة الثنائية لجميع حسابات مدراء النظام",
  },
  en: {
    ...
    selfRegDesc:
      "Allow hospitals and new donors to register accounts directly without review",
...
// --- معالجة رفع وحذف شعار المنصة (Logo) ---
...
const handleLogoChange = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      props.generalSettings.logoUrl = url;
      localStorage.setItem("musaef_logo_url", url);
    };
...
const deleteLogo = () => {
  ...
  props.generalSettings.logoUrl = null;
  localStorage.removeItem("musaef_logo_url");
  if (logoFileInput.value) logoFileInput.value.value = "";
};

// --- معالجة رفع وحذف أيقونة المتصفح (Favicon) ---
const triggerFaviconUpload = () => {
  if (faviconFileInput.value) faviconFileInput.value.click();
};
...
const handleFaviconChange = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      props.generalSettings.faviconUrl = url;

      // الحفظ دائماً في localStorage لضمان استمراريتها
      localStorage.setItem("musaef_favicon_url", url);

      // تحديث أيقونة Favicon الحقيقية بالصفحة أعلى المتصفح فوراً
...
const deleteFavicon = () => {
  ...
  props.generalSettings.faviconUrl = null;
  localStorage.removeItem("musaef_favicon_url");
  if (faviconFileInput.value) faviconFileInput.value.value = "";

  // إعادة الأيقونة الافتراضية للـ Document
  const defaultFavicon = getImageUrl("auth-logo.png");
  applyFaviconToDocument(defaultFavicon);
};

// --- معالجة تغيير لغة النظام الافتراضية ---
const onLanguageChange = (event) => {
  const selectedLang = event.target.value;
  localStorage.setItem("musaef_lang", selectedLang);
  window.dispatchEvent(new Event("musaef_lang_changed"));
};
</script>
