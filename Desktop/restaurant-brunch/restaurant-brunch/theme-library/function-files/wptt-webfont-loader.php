...
if ( ! class_exists( 'Restaurant_Brunch_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Restaurant_Brunch_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $restaurant_brunch_url The remote URL.
		 */
		public function __construct( $restaurant_brunch_url = '' ) {
			$this->remote_url = $restaurant_brunch_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $restaurant_brunch_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $restaurant_brunch_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $restaurant_brunch_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $restaurant_brunch_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $restaurant_brunch_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $restaurant_brunch_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $restaurant_brunch_format = 'woff2' ) {
			$this->font_format = $restaurant_brunch_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $restaurant_brunch_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $restaurant_brunch_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $restaurant_brunch_url, '/' ) ) {
				$restaurant_brunch_parsed_url = parse_url( $this->remote_url );
				return $restaurant_brunch_parsed_url['scheme'] . '://' . $restaurant_brunch_parsed_url['hostname'] . $restaurant_brunch_url;
			}

			return $restaurant_brunch_url;
		}
	}
}

if ( ! function_exists( 'restaurant_brunch_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param string $restaurant_brunch_url    The URL of the remote webfont.
	 * @param string $restaurant_brunch_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function restaurant_brunch_wptt_get_webfont_styles( $restaurant_brunch_url, $restaurant_brunch_format = 'woff2' ) {
		$restaurant_brunch_font = new Restaurant_Brunch_WPTT_WebFont_Loader( $restaurant_brunch_url );
		$restaurant_brunch_font->set_font_format( $restaurant_brunch_format );
		return $restaurant_brunch_font->get_styles();
	}
}

if ( ! function_exists( 'restaurant_brunch_wptt_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function restaurant_brunch_wptt_get_webfont_url( $restaurant_brunch_url, $restaurant_brunch_format = 'woff2' ) {
		$restaurant_brunch_font = new Restaurant_Brunch_WPTT_WebFont_Loader( $restaurant_brunch_url );
		$restaurant_brunch_font->set_font_format( $restaurant_brunch_format );
		return $restaurant_brunch_font->get_url();
	}
}